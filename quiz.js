...
let state = null; // { candidateId, picked, currentIndex, answers, timerEnd, flagsBuffer }
...
function pickQuestionsForAttempt() {
  const byDay = {};
  QUESTIONS.forEach((q, idx) => {
    (byDay[q.day] ||= []).push(idx);
  });
  let picked = [];
  Object.keys(byDay).sort((a, b) => a - b).forEach((day) => {
    const shuffled = shuffle(byDay[day]);
    picked = picked.concat(shuffled.slice(0, QUESTIONS_PER_DAY));
  });
...
export async function startAttempt(name) {
  const candidateId = `${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const picked = pickQuestionsForAttempt();
...
export async function submitAttempt() {
  ...
  await updateDoc(doc(db, "candidates", state.candidateId), {
    ...
    score,
    maxScore: state.picked.length * POINTS_PER_QUESTION,
    scoreByDay,
  ...
  return { score, maxScore: state.picked.length * POINTS_PER_QUESTION, scoreByDay };
}
